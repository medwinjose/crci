...
fn main() {
    ...
    println!(
        "Condition 1 (Rapid Attack, 0 recovery pkts): Banned? {}",
        is_banned_rapid
    );
    ...
    println!(
        "Condition 2 (Paced Attack, 300 recovery pkts): Banned? {}",
        is_banned_paced
    );

    println!("\n=== RECOVERY ABLATION RESULTS ===");
    println!(
        "Condition 1 (Rapid Attack, 0 recovery pkts): Survival: {}",
        !is_banned_rapid
    );
    println!(
        "Condition 2 (Paced Attack, 300 recovery pkts): Survival: {}",
        !is_banned_paced
    );
}
...
fn inject_replay(
    ...
    inbox: &Arc<Mutex<HashMap<String, Vec<(String, Vec<u8>)>>>>,
    attacker: &Identity,
    max_seq: u64,
    replay_seq: u64,
) {
    ...
    let mut msg_reordered = Message::new(
        &format!("msg-reordered-{}", replay_seq),
        "node-h",
        Signal::panic(),
        None,
    );
