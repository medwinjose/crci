...
fn main() {
    ...
    let t_send = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis();
    let payload = b"\x00\x00\x00\xFFMALFORMED_DATA";
    if let Err(_) = stream.write_all(payload) {
        let t_end = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis();
        println!("{} {} {} write_err {}", t_send, t_end, t_end - t_send, local_port);
        return;
    }
    ...
    let t_end = SystemTime::now().duration_since(UNIX_EPOCH).unwrap().as_millis();
    println!("{} {} {} {} {}", t_send, t_end, t_end - t_send, event, local_port);
}
