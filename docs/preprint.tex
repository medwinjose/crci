...
\title{CRCI: Reputation-Based Local Misbehaving-Peer Isolation for Mesh Communication}
...
\begin{enumerate}
    ...
    \item We design and implement the peer-to-peer communication stack and fault-isolation mechanism in Rust.
...
\subsection{Emergency Decision Architecture (EDA)}
...
\begin{table}[htbp]
\caption{Strikes to Eviction and Processing Latency Across 50 Trials}
\label{tab:latency}
\centering
\begin{tabular}{lcccc}
\toprule
...
Fixed 15ms Delay & 3 & 45.82 ms & 45 ms / 46 ms & 50/50 \\
\bottomrule
\end{tabular}
\end{table}
...
We compared CRCI's sliding-window replay policy ($W=64$) against a naive strict-monotonic rule (\texttt{seq <= last\_seq}). Both policies were evaluated over 50 trials using the same telemetry workload, reputation parameters, and controlled packet-reordering pattern (depth of 2 packets) for the honest peer:
\begin{itemize}
    \item \textbf{Strict-Monotonic Baseline:} The out-of-order packet was immediately classified as a replay, resulting in an unjust penalty. Repeated reordering led directly to false-positive bans (Honest Survival = 0/50), as expected by construction because three reordered packets trigger a ban under the strict-monotonic policy.
    \item \textbf{CRCI Sliding Window:} The out-of-order packet was correctly accepted. The honest peer remained completely unharmed (Honest Survival = 50/50).
\end{itemize}
