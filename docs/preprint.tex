...
CRCI integrates a local misbehaving-peer isolation mechanism directly into an epidemic gossip protocol. Unlike PBFT, CRCI does not attempt to reach globally synchronized consensus. Unlike federated consensus mechanisms such as Stellar SCP \cite{mazieres2015stellar} and the Ripple Protocol \cite{schwartz2014ripple}, CRCI does not establish distributed agreement through quorum-based voting. By binding reputation metrics to authenticated identities, CRCI enforces a dynamic fault-isolation threshold via a ``3-strikes'' penalty system without synchronous voting rounds.
...
\begin{tabular}{lccc}
\toprule
\textbf{Adversarial Behavior} & \textbf{System response} & \textbf{Penalized?} & \textbf{Evicted?} \\
\midrule
...
0.5 - 3(0.15) = 0.05 < 0.1
\end{equation}
After 3 explicitly malicious acts attributable to the same transport peer, that peer identity is banned locally and subsequent connections using that identity are rejected. The transport layer tracks \texttt{already\_sent} IDs per neighbor. The per-neighbor sent-ID cache prevents the sender-side gossip scheduler from repeatedly forwarding the same payload to a peer during periodic dissemination, while receiver-side cross-path duplicates remain harmless. Peers whose reputation is diminished but not banned slowly recover reputation over time through a \texttt{decay\_recover} mechanism (e.g., $+0.05$ per hour). Rate limit violations result in dropped messages without reputation penalties.
...
\section{Protocol Implementation and Functional Validation}
\label{sec:results}
